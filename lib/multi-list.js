'use babel'

export default class MultiList {
  constructor (lists) {
    this.lists = lists
    this.selectedListIndex = 0
    this.activeItemIndexForList = Array(lists.length).fill(0)
    this.selectedItemIndexSetForList = Array(lists.length).fill(new Set())
  }

  getListAtIndex (listIndex) {
    return this.lists[listIndex]
  }

  selectListAtIndex (listIndex) {
    this.selectedListIndex = listIndex
  }

  getSelectedListIndex () {
    return this.selectedListIndex
  }

  selectItemAtLocation ([listIndex, itemIndex], {isCollection, selectIntermediate}) {
    this.selectListAtIndex(listIndex)
    if (isCollection) {
      this.addSelectedItemIndexForList(listIndex, itemIndex)
    }
    this.setActiveItemIndexForList(listIndex, itemIndex)
  }

  getItemAtLocation ([listIndex, itemIndex]) {
    return this.lists[listIndex][itemIndex]
  }

  setActiveItemIndexForList (listIndex, itemIndex) {
    this.activeItemIndexForList[listIndex] = itemIndex
  }

  getActiveItemIndexForList (listIndex) {
    if (listIndex === undefined) throw new RangeError(`listIndex ${listIndex} out of range`)
    return this.activeItemIndexForList[listIndex]
  }

  getSelectedItemIndexSetForList (listIndex) {
    if (listIndex === undefined) throw new RangeError(`listIndex ${listIndex} out of range`)
    return this.selectedItemIndexSetForList[listIndex]
  }

  getActiveItemForList (listIndex) {
    if (listIndex === undefined) throw new RangeError(`listIndex ${listIndex} out of range`)
    return this.lists[listIndex][this.getActiveItemIndexForList(listIndex)]
  }

  getSelectedItemSetForList (listIndex) {
    if (listIndex === undefined) throw new RangeError(`listIndex ${listIndex} out of range`)
    const indexSet = this.lists[listIndex][this.getSelectedItemIndexSetForList(listIndex)]
    return new Set([...indexSet].map(i => this.getItemAtLocation([listIndex, i])))
  }

  getActiveItem () {
    return this.getActiveItemForList(this.selectedListIndex)
  }

  getSelectedItems () {
    return this.getSelectedItemSetForList(this.selectedListIndex)
  }

  getItemLocationFromFlattenedIndex (flattenedIndex) {
    if (flattenedIndex < 0) throw new RangeError(`Index ${flattenedIndex} out of range`)
    const totalLength = this.lists.reduce((totalLength, list) => totalLength + list.length, 0)
    if (totalLength < flattenedIndex) throw new RangeError(`Index ${flattenedIndex} out of range`)

    let listLengths = this.lists.map(list => list.length)
    let count = 0
    let currentListIndex = 0
    while (count + listLengths[currentListIndex] <= flattenedIndex) {
      count += listLengths[currentListIndex]
      currentListIndex++
    }
    let localItemIndex = flattenedIndex - count
    return [currentListIndex, localItemIndex]
  }

  selectItem (item) {
    const flattenedList = this.lists.reduce((acc, list) => {
      return acc.concat(list)
    }, [])
    const flattenedIndex = flattenedList.indexOf(item)
    this.selectItemAtLocation(this.getItemLocationFromFlattenedIndex(flattenedIndex))
  }

  moveListSelection (step) {
    if (step !== 1 && step !== -1) {
      throw new Error(`invalid step size ${step}. must be 1 or -1`)
    }
    let newListIndex = this.selectedListIndex + step
    if (newListIndex >= 0 && newListIndex < this.lists.length) {
      this.selectListAtIndex(newListIndex)
    }
  }

  moveItemSelection (step) {
    if (step !== 1 && step !== -1) {
      throw new Error(`invalid step size ${step}. must be 1 or -1`)
    }
    const listIndex = this.selectedListIndex
    const newItemIndex = this.getSelectedItemIndexForList(listIndex) + step
    if (newItemIndex >= 0 && newItemIndex < this.lists[listIndex].length) {
      this.selectItemAtLocation([listIndex, newItemIndex])
    }
  }

  updateLists (newLists) {
    if (newLists.length !== this.lists.length) {
      throw new Error('Number or lists not equal')
    }
    for (let listIndex = 0; listIndex < newLists.length; listIndex++) {
      const itemIndex = this.selectedItemIndexSetForList[listIndex]
      const item = this.lists[listIndex][itemIndex]
      const newIndex = newLists[listIndex].indexOf(item)
      if (newIndex > -1) {
        // if item is still present in list, return its new index
        this.selectedItemIndexSetForList[listIndex] = newIndex
      } else if (newLists[listIndex].length > itemIndex) {
        // else item not present, but if there is another item in the same location, return current index
        this.selectedItemIndexSetForList[listIndex] = itemIndex
      } else if (newLists[listIndex].length) {
        // else nothing at location, if there are still items in the current list, return last item
        this.selectedItemIndexSetForList[listIndex] = newLists[listIndex].length - 1
      } else {
        if (this.getSelectedListIndex() === listIndex) {
          // else list is empty, and if current list is the selected list...
          if (newLists[listIndex + 1] && newLists[listIndex + 1].length) {
            // ... and there is an element in the next list, set the first item as selected
            this.selectItemAtLocation([listIndex + 1, 0])
          } else if (newLists[listIndex - 1] && newLists[listIndex - 1].length) {
            // if the next list is empty but the previous list is non-empty, set the last item as selected
            this.selectItemAtLocation([listIndex - 1, newLists[listIndex - 1].length - 1])
          }
          this.selectedItemIndexSetForList[listIndex] = 0
        }
      }
    }
    this.lists = newLists
  }

  toObject () {
    const {lists, selectedListIndex, selectedItemIndexSetForList} = this
    return {lists, selectedListIndex, selectedItemIndexSetForList}
  }
}
