...
export default class GithubPackage {
  constructor(workspace, project, commandRegistry, notificationManager, tooltips, styles, grammars, confirm, config,
    ...
    this.subscriptions = new CompositeDisposable(
      ...
      workspace.openers.add(({uri}) => {
        if (uri.matches) {
          const item = StubItem.create('github-FilePatchView', uri);
          this.openFilePatchViews.push({
            uri,
            stubItemSelector: item,
          });
          this.rerender();
          return null;
        }
  ...
  rerender(callback) {
    ...
    ReactDom.render(
      <RootController
        openFilePatchViews={this.openFilePatchViews}
  ...
  @autobind
  createFiklePatchControllerStub({serializedState}) {
    return StubItem.create('github-FilePatchView', serializedState.uri, {
      ...
    })
    this.openFilePatchViews.push({uri, item: stubItem})
    if (this.activated) this.rerender();
  }
