...
export class IssueishPaneItemView extends React.Component {
    ...
    return (
      <div className="github-PrPaneItem native-key-bindings">
        <div className="github-PrPaneItem-container">
          ...
          <div>
            <input type="text" ref={c => { this.inputBox = c; }} />
  ...
  handleButtonClicked(e) {
    const variables = {
      input: {
        subjectId: this.props.issueish.id,
        body: this.inputBox.value,
      },
    };
    ...
    commitMutation(
      this.props.relay.environment,
      {
          ...
          addComment: {
            commentEdge: {
              node: {
                author: {
                  __typename: 'User',
                  avatarUrl: 'https://avatars0.githubusercontent.com/u/189606?v=4',
                  login: 'BinaryMuse',
                },
                bodyHTML: this.inputBox.value,
                createdAt: new Date(),
              },
            },
          },
        ...
        configs: [
          {
            type: 'RANGE_ADD',
            parentID: this.props.issueish.id,
            connectionInfo: [{
...
export default createRefetchContainer(IssueishPaneItemView, {
  ...
  issueish: graphql`
    fragment IssueishPaneItemContainer_issueish on IssueOrPullRequest {
      ...
      ... on Issue {
        id state number title bodyHTML
        author {
          login avatarUrl
          ... on User { url }
          ... on Bot { url }
