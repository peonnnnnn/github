...
export default class MultiList {
  ...
  selectNextItem () {
    ...
    } else if (listIndex < this.lists.length - 1) {
      this.selectedListIndex = listIndex + 1 // Avoid duplicate didChangeSelection notifications by assigning directly
  ...
  selectPreviousItem () {
    ...
    if (newItemIndex >= 0) {
      ...
      this.selectedListIndex = listIndex - 1 // Avoid duplicate didChangeSelection notifications by assigning directly
      this.selectItemAtLocation([listIndex - 1, this.lists[listIndex - 1].length - 1])
    }
  }
