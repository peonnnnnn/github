...
export default class FileDiff {
  ...
  async calculateHunks(hunks, stagedLines, fileName, stageFn) {
    const resultHunks = []
    let lineIndex = 0
    let lineOffset = 0
    for (const hunk of hunks) {
      ...
      await diffHunk.fromGitUtilsObject({hunk, stagedLines: containedLines, fileName, stageFn})
  ...
  async fromGitUtilsObject({diff, stagedDiff, stageFn}) {
    ...
    hunks = await this.calculateHunks(hunks, stagedLines, diff.oldFile().path(), stageFn)
