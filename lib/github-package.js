...
import ChangedFilesCountController from './controllers/changed-files-count-controller'
...
export default class GithubPackage {
  constructor (workspace, project, commandRegistry, notificationManager) {
    ...
    this.changedFilesCountController = new ChangedFilesCountController({
      repository: null, didClick: this.toggleGitPanel.bind(this)
    })
  ...
  async activate (state = {}) {
    ...
    if (state.filePatch) {
      const status = state.filePatch.stagingStatus
      const filePath = state.filePatch.filePath
      const repo = this.getActiveRepository()
      let filePatch
      if (status === 'staged') {
        filePatch = repo.stagedFilePatchesByPath.get(filePath)
      } else {
        filePatch = repo.unstagedFilePatchesByPath.get(filePath)
      }
      if (filePatch) {
        this.didSelectFilePatch(filePatch, status)
      }
    }
  }

  serialize () {
    const ctrl = this.filePatchController
    return {
      gitPanelVisible: true,
      filePatch: ctrl ? {
        stagingStatus: ctrl.props.stagingStatus,
        filePath: ctrl.props.filePatch.getPath()
      } : null
    }
  }
  ...
  consumeStatusBar (statusBar) {
    const tile = statusBar.addRightTile({item: this.changedFilesCountController, priority: -100})
  ...
  async updateActiveRepository () {
    ...
    if (activeRepository && activeRepository !== this.activeRepository) {
      ...
      await this.changedFilesCountController.update({repository: this.getActiveRepository()})
