...
export default class RootController extends React.Component {
  ...
  render() {
    return (
      <div>
        ...
        {this.renderPanels()}
        {(this.state.diffViewOpen && this.state.filePath && this.state.filePatch) ? this.renderFilePatchController() : null}
        ...
        <DockItem
          ...
          <EtchWrapper
            ...
            <GitTabController
              ...
              didSelectFilePath={this.didSelectFilePath}
              ...
              activateDiffView={this.activateDiffView}
              didCommit={this.didCommit}
            />
          </EtchWrapper>
        </DockItem>
      );
        ...
        <PaneItem
          ...
          <FilePatchController
            ref={c => {
              ...
              c && this.filePatchController.onDidDestroy(() => {
                this.deactivateDiffView();
              });
            }}
  ...
  @autobind
  activateDiffView() {
    this.setState({diffViewOpen: true});
  }

  @autobind
  deactivateDiffView() {
    this.setState({diffViewOpen: false});
  }

  @autobind
  didCommit() {
    console.log('stage status', this.filePatchController.getStageStatus());
    if (this.filePatchController.getStageStatus() === 'staged') {
  ...
  @autobind
  didSelectFilePath(filePath, stagingStatus, options = {}) {
    if (this.state.diffViewOpen && filePath && this.props.repository) {
      return this.showFilePatchForPath(filePath, stagingStatus, options);
    } else {
      return null;
    }
  }
  ...
  async showFilePatchForPath(filePath, stagingStatus, {activate, amending, lineNumber} = {}) {
    // if (!this.state.diffViewOpen || !filePath || !repository) { return null; }
