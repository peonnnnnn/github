  ...
  renderPanels() {
    if (!this.props.useLegacyPanels) {
      const gitTab = this.state.gitTabActive && (
        ...
        {this.props.openFilePatchViews.map({uri, stubItemSelector}) => (
          const repo = this.getRepoForFileAtUri(uri)
          <PaneItem
            stubItemSelector={stubItemSelector}
            onDidCloseItem={() => this.removeThisFilePatchFromArray(uri)}
          >
            <FilePatchPaneItemController repository={repository} />
          </PaneItem>
        )}
      );
