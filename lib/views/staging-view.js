...
export default class StagingView {
  ...
  async update(props) {
    ...
    // TODO: why is this causing the diff view to show up any time a file is clicked in tree view????
    // Because the whole damn tree is rerendering since the staged/unstaged changes are different
    // the purpose of this change was to update after a partially staged file is totally staged
    // find a better fix...
    // I think this is happening because debounce... triggers showFilePatch which focuses the file patch...
  ...
  stageAll() {
    ...
    return this.attemptFileStageOperation(filePaths, 'unstaged');
  }
  ...
  unstageAll() {
    ...
    return this.attemptFileStageOperation(filePaths, 'staged');
  }
  ...
  confirmSelectedItems() {
    ...
    this.attemptFileStageOperation(itemPaths, this.selection.getActiveListKey());
  ...
  @autobind
  didChangeSelectedItems(activateDiffView) {
    const selectedItems = Array.from(this.selection.getSelectedItems());
    if (selectedItems.length === 1) {
      if (activateDiffView) { this.props.activateDiffView(); }
  ...
  dblclickOnItem(event, item) {
    return this.attemptFileStageOperation([item.filePath], this.selection.listKeyForItem(item));
  }

  attemptFileStageOperation(filePaths, stageStatus) {
    this.props.attemptFileStageOperation(filePaths, stageStatus);
    this.selection.coalesce();
    this.debouncedDidChangeSelectedItem();
    return etch.update(this);
  }
